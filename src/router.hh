...
private:
    ...
    struct TrieNode {
        ...
        std::optional<RTE> REntry{};
    };

    std::shared_ptr<TrieNode> root{std::make_shared<TrieNode>()};

    std::optional<RTE> bestMatch(uint32_t dst_ip_address) {
        std::optional<RTE> bestmatch;
        std::shared_ptr<TrieNode> ptr = root;
        for (int i = 0; i < 32; i++) {
            bool bit = (dst_ip_address & (1 << (31 - i))) != 0;
            if (ptr->REntry.has_value())
                bestmatch = ptr->REntry;
            if (!ptr->children[bit])
                break;
            else
